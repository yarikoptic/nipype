...
from nipype.interfaces.base import (CommandLineInputSpec, CommandLine,
                                    traits, TraitedSpec, File, InputMultiPath)
...
class ANTSInputSpec(CommandLineInputSpec):
    ...
    mask_file = File(exists=True, argstr='--mask-image %s',
        desc='this mask -- defined in the fixed image space defines the region '
             'of interest over which the registration is computed ==> above '
             '0.1 means inside mask ==> continuous values in range [0.1,1.0] '
    ...
    fixed_image_initial_affine  = File(
        exists=True, argstr='--fixed-image-initial-affine %s',
        desc='Use the input file as the initial affine parameter '
             'for the fixed image')

    geodesic = traits.Enum(
        'not time-dependent', 'asymmetric', 'symmetric', argstr='--geodesic %s',
        desc='Geodesic; can be: not time-dependent, asymmetric, or symmetric')

    go_faster = traits.Bool(argstr='--go-faster',
        desc='true / false -- if true, SyN is faster but loses some accuracy '
              'wrt inverse-identity constraint, see Avants MIA 2008. '
              '<VALUES>: false')

    continue_affine  = traits.Bool(argstr='--continue-affine',
        desc='true (default) | false, do (not) perform affine given the '
             'initial affine parameters <VALUES>: true')

    number_of_affine_iterations = traits.List(
        traits.Int, argstr='--number-of-affine-iterations %d',
    ...
    affine_metric_type  = traits.Enum(
        'MI', 'MSQ', 'CC', 'GD', argstr='--affine-metric-type %s',
        desc= 'MI: mutual information (default), MSQ: mean square error,'
        'CC: Normalized correlation, '
        'CCH: Histogram-based correlation coefficient (not recommended), '
        'GD: gradient difference (not recommended) '
        '<VALUES>: MI')

    mutual_info_options = traits.List(traits.Int, argstr='--MI-option %d',
        desc='option of mutual information: '
        'MI_bins x MI_samples (default: 32x32000) '
    ...
    affine_gradient_descent_options = traits.List(
        traits.Float, argstr='--affine-gradient-descent-option  %d',
    ...
    out_file = File(argstr='-o %s', genfile=True,
        desc='The name for the output: prefix or a name+type, '
             'e.g. OUT or OUT.nii or OUT.mha')

    image_dimension = traits.Enum(3, 2,
        argstr='%d', mandatory=True, position=1, usedefault=True,
...
class ANTS(CommandLine):
    ...
    SMI/spatial-mutual-information/SpatialMutualInformation
        [fixedImage,movingImage,weight,radius/OrForMI-#histogramBins]
    PR/probabilistic/Probabilistic
        [fixedImage,movingImage,weight,radius/OrForMI-#histogramBins]
    ...
    PSE/point-set-expectation/PointSetExpectation
        ...
        the partial matching option assumes the complete labeling is in
        the first set of label parameters ... more iterations leads to
        more symmetry in the matching - 0 iterations means full asymmetry
    ...
    def _format_arg(self, name, spec, value):
        if name == 'geodesic':
            return spec.argstr % {"not time-dependent":'0',
                                  "asymmetric":'1',
                                  "symmetric":'2'}[value]
...
class ANTS_SpatialMutualInformation(ANTS_Intensity):
    ...
    def __init__(self, command=None, **inputs):
        ...
        super(ANTS_SpatialMutualInformation, self).__init__(command, **inputs)
...
class ANTS_Probabilistic(ANTS_Intensity):
    ...
    def __init__(self, command=None, **inputs):
        ...
        super(ANTS_Probabilistic, self).__init__(command, **inputs)
...
class ANTS_MeanSquares(ANTS_Intensity):
    ...
    def __init__(self, command=None, **inputs):
        ...
        super(ANTS_MeanSquares, self).__init__(command, **inputs)
...
class ANTS_PointSetExpectation(ANTS_PointSet):
    ...
    def __init__(self, command=None, **inputs):
        ...
        super(ANTS_PointSetExpectation, self).__init__(command, **inputs)
...
class ANTS_JensenTsallisBSpline(ANTS_PointSet):
    ...
    def __init__(self, command=None, **inputs):
        ...
        super(ANTS_JensenTsallisBSpline, self).__init__(command, **inputs)
...
"""
    ...
    ## Need to write xor for reference image for the
    ## tightest-bounding-box and reslice-by-header booleans
    compute_tightest_bounding_box = traits.Bool(
        argstr='--tightest-bounding-box',
        desc='Computes the tightest bounding box using all the affine '
        'transformations. It will be overrided by -R reference_image if given.')

    reslice_by_header = traits.Bool(argstr='--reslice-by-header',
        desc='equivalent to -i -mh, or -fh -i -mh if used together with the '
             'reference_image. It uses the orientation matrix and origin '
             'encoded in the image file header. It can be used together '
             'with reference_image. This is typically not used together with '
...
class AverageImagesInputSpec(CommandLineInputSpec):
    ...
    normalize  = traits.Bool(argstr='%s', position=3,
        ...
        'if true, the 2nd image is divided by its mean. '
